...
以評測集（`registry/eval-queries.json`，47 筆）量化，逐項驗證後才採用：

| 指標 | 改善前 | 改善後 |
|------|-------|-------|
| agent 引擎 Hit@1 | 11.9% | **38.1%** |
| fusion 引擎 Hit@1 | 7.1% | **38.1%** |
| 加上 LLM rerank | 43–48% | **73.8%** |
| 空集誠實率 | 80% | **100%** |
...
**方法論**：所有改動均先診斷再實作，且每批擴充後重新量測。過程中三次「先診斷」都改變了原定方向——例如原本要補齊缺欄位，診斷後發現瓶頸其實是語意鴻溝。
...
```bash
npm test                          # 單元測試（無外部依賴，項數以實際輸出為準）
node scripts/check-syntax.js      # 語法守門：全部 .js 跑 node --check
...
> 因此可在離線與沙盒環境中穩定執行。需要 `npx skills` CLI 與 GitHub API 的整合測試
> 另以 `npm run test:integration` 執行（項數以實際輸出為準）。
