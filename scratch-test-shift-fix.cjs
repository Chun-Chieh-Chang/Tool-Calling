const { chromium } = require('playwright');
const path = require('path');

(async () => {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport: { width: 1920, height: 1080 } });

  const htmlPath = path.resolve(__dirname, 'docs/knowledge-graph.html');
  const fileUrl = 'file:///' + htmlPath.replace(/\\/g, '/');
  await page.goto(fileUrl, { waitUntil: 'networkidle' });
  await page.waitForTimeout(2000);

  // Switch to 3D
  await page.click('#viewToggleBtn');
  await page.waitForTimeout(3000);

  // Inject pointerdown capture phase listener for Shift+Left Click Pan and contextmenu preventDefault
  await page.evaluate(() => {
    const container3d = document.getElementById('network3d');
    container3d.addEventListener('contextmenu', e => e.preventDefault());

    container3d.addEventListener('pointerdown', function(e) {
      const controls = graph3DInstance.controls();
      if (!controls || !controls.mouseButtons) return;
      if (e.button === 0) { // Left click
        if (e.shiftKey) {
          controls.mouseButtons.LEFT = 2; // PAN
        } else {
          controls.mouseButtons.LEFT = 0; // ROTATE
        }
      }
    }, true); // CAPTURE PHASE!
  });

  const getCameraState = () => page.evaluate(() => {
    const c = graph3DInstance.camera();
    const t = graph3DInstance.controls().target;
    return {
      camPos: { x: c.position.x, y: c.position.y, z: c.position.z },
      targetPos: { x: t.x, y: t.y, z: t.z }
    };
  });

  // Test 1: Shift + Left Click Drag
  await page.keyboard.down('Shift');
  await page.mouse.move(960, 540);
  const startState = await getCameraState();
  
  await page.mouse.down({ button: 'left' });
  await page.mouse.move(1160, 540, { steps: 10 });
  await page.mouse.up({ button: 'left' });
  await page.keyboard.up('Shift');
  await page.waitForTimeout(500);

  const endStateShift = await getCameraState();
  console.log('Before Shift+Left:', startState);
  console.log('After Shift+Left:', endStateShift);

  const targetMovedShift = Math.abs(endStateShift.targetPos.x - startState.targetPos.x) > 5;
  console.log('Did Shift + Left Drag PAN (target moved)?', targetMovedShift ? 'SUCCESS (PAN)' : 'FAILED (ROTATE)');

  // Test 2: Normal Left Click Drag (should ROTATE)
  await page.evaluate(() => graph3DInstance.cameraPosition({ x: 0, y: 0, z: 480 }, { x: 0, y: 0, z: 0 }, 0));
  await page.waitForTimeout(500);
  const startStateNormal = await getCameraState();

  await page.mouse.move(960, 540);
  await page.mouse.down({ button: 'left' });
  await page.mouse.move(1160, 540, { steps: 10 });
  await page.mouse.up({ button: 'left' });
  await page.waitForTimeout(500);

  const endStateNormal = await getCameraState();
  console.log('Before Normal Left:', startStateNormal);
  console.log('After Normal Left:', endStateNormal);

  const camRotated = Math.abs(endStateNormal.camPos.x - startStateNormal.camPos.x) > 5;
  console.log('Did Normal Left Drag ROTATE (camPos.x changed)?', camRotated ? 'SUCCESS (ROTATE)' : 'FAILED');

  await browser.close();
})();
