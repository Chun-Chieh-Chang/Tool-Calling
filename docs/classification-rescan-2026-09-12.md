...
| 分層 | 數量 | 說明 |
|---|---|---|
| **Tier 1 — 明確規則違反** | **1** | 決策樹有明文規則適用，現行分類不符 → 建議套用 |
| Tier 2 — 需人工裁決 | 18 | 決策樹與既有慣例衝突，或規則依賴語境、誤判率偏高 → 不自動套用 |
| Tier 3 — 領域關鍵詞啟發 | 20 | 決策樹 §2.1 關鍵詞表在**啟發級**（含 triggers）命中 → 僅供參考，不自動套用 |
| 合規（規則命中且分類正確） | 199 | 已符合決策樹 |
| 無明確規則命中 | 464 | 決策樹未涵蓋，**維持現狀（非違規）** |

**規則命中者合規率**：199 / 238 = 83.6%
**決策樹覆蓋率**：238 / 702 = 33.9%
...
| 工具 | ⭐ | 現行分類 | 建議分類 | 命中依據 |
|---|---:|---|---|---|
| `lint`<br><sub>Lint</sub> | 0 | AI 代理 | **開發工具** | `identity:lint|linter` |
