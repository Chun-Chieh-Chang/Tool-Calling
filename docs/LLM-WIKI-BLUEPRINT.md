...
| **PPR 圖譜擴散演算法（第 7 頁）** | ✅ **已實作**（2026-09-20）：圖譜貢獻從 +0.7pp → **+2.6pp**，top1 60.4% → **62.3%** |
| **擷取層管線（第 3、4 頁）** | ✅ **已實作**：`core/ingestion.js`，五種素材來源，置信度不足就明說找不到 |
| **Epistemic Markers（第 10 頁）** | ✅ **已實作**：Tier 0→`[V]`、Tier 1→`[S]`，既有 705 筆已補標 |
| Prompt 工程（第 11 頁）| ❌ **實測無效**：「先排除再挑選」比直接挑選差 3.9pp（詳見 §11） |
| 模型路由（第 9 頁）| 未做（批次作業可改用便宜模型，低優先） |
...
標題：**LLM Wiki 建構藍圖 — 2026 個人知識庫的典範轉移：「手動整理」到「AI 自動編譯」**
...
### → 對本專案的可行動結論（**已實作，2026-09-20**）

我們原本做的是「詞—詞共現圖 + **一階**鄰居擴散」，實測只貢獻 +0.7pp——
原因正是**只走一階**：共現邊太稀疏（355 節點／264 邊），幾乎擴不出去。
...
|---|---|---|---|
| 無圖譜 | 96.9% | 59.7% | — |
| 一階共現（舊）| 96.9% | 60.4% | +0.7pp |
| **PPR（現行）** | **96.9%** | **62.3%** | **+2.6pp** |

🔴 實作時發現一個原本的 bug：**既有的 `GRAPH_DECAY` 完全沒作用**。
`weightedSim()` 只看 token 有沒有出現、不看權重，所以衰減係數形同裝飾。
PPR 必須搭配新的 `weightedCosine()`（查詢端帶權重）才吃得到距離衰減。

→ 這也說明：**「照著論文概念做」跟「概念真的生效」是兩件事**，
  中間要靠可量測的消融實驗（`npm run ablate:v5`）把落差抓出來。
...
- **放開解釋（CoT）**：rerank 實測 semantic 類型完全沒動、整體略降
  （78.6% → 76.2%），故維持禁止解釋。
- **先排除再挑選**（2026-09-20 實測，CoT 的輕量版）：
  只請模型列出明顯不相關的候選（EXCLUDE），再從剩下的挑（PICK）。

  | 策略 | Hit@1 |
  |---|---|
  | 直接挑選（現行）| **84.4%** |
  | 先排除再挑選 | 80.5%（−3.9pp，p=0.227）|

  ❌ 不採用。額外缺點：輸出排除清單讓 token 變多 → 成功率掉到 86%
  （低於 90% 的可靠門檻，其他實驗都有 98%+）。

→ 兩次嘗試都失敗，推測差異來自任務性質：它們是**問答**（需要推理鏈），
  我們是**挑選**（需要的是辨識力，不是推論過程）。
