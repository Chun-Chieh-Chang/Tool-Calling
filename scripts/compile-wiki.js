...
const GENERIC_PATTERNS = [
  ...
  /^帮我(搞定|看看|想想|处理|弄|做|想辦法)/, // allow-simplified：刻意引用的簡體關鍵字／範例，轉繁會破壞比對
  /有(沒有|没有)?(什麼|什么)?(工具|app|APP|套件|軟體)/, // allow-simplified：刻意引用的簡體關鍵字／範例，轉繁會破壞比對
  /(求|給我)?推薦/,
  /怎麼(實現|实现|搞|做)$/, // allow-simplified：刻意引用的簡體關鍵字／範例，轉繁會破壞比對
  /^(這個|这个)/, // allow-simplified：刻意引用的簡體關鍵字／範例，轉繁會破壞比對
  /(資料|数据|檔案|文件|東西|东西)$/,   // 結尾只有泛稱 → 沒有具體對象 // allow-simplified：刻意引用的簡體關鍵字／範例，轉繁會破壞比對
];
