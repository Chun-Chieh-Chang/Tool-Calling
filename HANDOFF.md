...
| **rerank 後（嚴格）** | **78.6%** ⚠️ | 三輪 79.9/78.0/78.0（成功呼叫 156/158/156）；**無法判定與基線的差異** |
| **rerank 後（含近義）** | **80.5%** | 歷史基線 81.8% |
| 空集誠實率 | 100%（10/10）| V5 啟用前後皆為 100% |

⚠️ **rerank 路徑的量測目前無法下定論**（2026-09-20）：

| 組別 | 嚴格 Hit@1 | 成功呼叫 |
|---|---|---|
| V5 啟用（Run 1/2/3） | 79.9% / 78.0% / 78.0%（平均 **78.6%**） | 157 / 158 / 156 |
| V5 停用（本日末輪） | **73.0%** | 153 |
| V5 停用（導入前歷史值） | 79.2% | — |

🔴 **73.0% 這個數字不可直接採信**：它是在連續跑了 4 輪評測之後才跑的，
命中本專案記錄過的**配額遞減順序效應**（見「換模型必須正反順序各跑一次」）。
成功呼叫數 153 vs 156~158 也印證配額確實比較吃緊。
而導入前的歷史基線是 79.2%，與 V5 的 78.6% 幾乎相同。

→ 保守結論：**rerank 路徑沒有可證實的變化**（最有可能是中性）。
若要定論，必須做**交替進行的配對 A/B**（同一批查詢內 V5 開／關交錯），
不能用「先跑三輪 A 再跑一輪 B」的順序設計。

可以確定的是：候選池確實變好了（top-20 天花板 89.3% → 90.6%），
但 LLM 在有答案的候選中挑對的比例沒有提升。這與既有結論一致——
**rerank 的瓶頸是 LLM 挑選力，不是召回**（見 `core/llm-rerank.js` 註解）。
